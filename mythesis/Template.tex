...
In general one can think about using different functions for the parametrization, since the only requirement is that shape of the estimator distributions is well described. There is no need for physical motivation of the selected functional forms. In terms of this master project, also fits with different functions have been performed. For the parameterization of the $m_{\text{W}}^{\rm reco}$ distribution for example, a Breit-Wigner distribution has been shown to describe the  distribution as well as the previous one.  An alternative suggestion for the  $m_{\text{top}}^{\rm reco}$ estimator might be the so-called crystall-ball function.  
Despite this possibility, main goal for this theses has been to be as consistent as possible with the previous measurements in order to make it easy to results.  
